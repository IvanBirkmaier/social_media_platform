...
create_k3d_cluster: ## baut das k3d Cluster.
    # Erstellen von einem Speicherplatz für Persistant Speicher
    mkdir -p "${PWD}"/kubernetes/k3dvol 
    # Cluster wird erstellt. Es wird die Registry übergeben und der Speicherort
    k3d cluster create social_media_cluster -v "${PWD}"/kubernetes/registry/registries.yaml:/etc/rancher/k3s/registries.yaml -v "${PWD}"/kubernetes/k3dvol:/tmp/k3dvol

.PHONY: delete_k3d_cluster
delete_k3d_cluster: ## delete das k3d Cluster.
    k3d cluster delete social_media_cluster
    # Löscht den Persistant Speicher
...
start_docker_registry: ## startet eine Docker Registry, welche im k3d Netzwerk sichtbar ist.
    docker container run -d --network k3d-social_media_cluster --name registry --restart always -p 5000:5000 registry:2
